...
_CASH_DRAWER = lambda m, t1='', t2='': ESC + b'p' + m + bytes([t1]) + bytes([t2])
CD_KICK_DEC_SEQUENCE = lambda esc, p, m, t1=50, t2=50: bytes([esc]) + bytes([p]) + bytes([m]) + bytes([t1]) + bytes([t2])
...
_PANEL_BUTTON = lambda n: ESC + b'c5' + bytes([n])
...
_SET_BARCODE_TYPE = lambda m: GS + b'k' + bytes([m])
