...
#  -*- coding: utf-8 -*-
""" Main class
Sourced for a large part from: https://github.com/python-escpos, and modified to work on upython
"""
...
# upython does not have a notion of current directory, so import otherwise
...
class SerialEscPos():
    ...
    def set(self, align='left', font='a', bold=False, underline=0, width=1,
        ...
        if custom_size:
            if 1 <= width <= 8 and 1 <= height <= 8 and isinstance(width, int) and\
              isinstance(height, int):
                ...
                self.raw(TXT_SIZE + bytes([size_byte]))
            else:
                raise SetVariableError()
        else:
            self.raw(TXT_NORMAL)
            if double_width and double_height:
                self.raw(TXT_STYLE['size']['2x'])
            elif double_width:
                self.raw(TXT_STYLE['size']['2w'])
            elif double_height:
                self.raw(TXT_STYLE['size']['2h'])
            else:
                self.raw(TXT_STYLE['size']['normal'])

        self.raw(TXT_STYLE['flip'][flip])
        ...
        if density != 9:
            self.raw(TXT_STYLE['density'][density])

        self.raw(TXT_STYLE['invert'][invert])
    ...
    def line_spacing(self, spacing=None, divisor=180):
        ...
        self.raw(LINESPACING_FUNCS[divisor] + bytes([spacing]))
    ...
    def cut(self, mode='FULL', feed=True):
        ...
        if not feed:
            self.raw(GS + b'V' + bytes([66]) + b'\x00')
            return
    ...
    def cashdraw(self, pin):
        ...
        if pin == 2:
            ...
            try:
                self.raw(CD_KICK_DEC_SEQUENCE(*pin))
        ...
        if ctl.upper() == "LF":
            ...
            if not (0 <= count <= 32 and
                ...
                for iterator in range(1, count):
                    self.raw(bytes([iterator * tab_size]))
                self.raw(NUL)
        elif ctl.upper() == "VT":
            self.raw(CTL_VT)
    ...
    def panel_buttons(self, enable=True):
        ...
        """
        if enable:
            self.raw(PANEL_BUTTON_ON)
        else:
            self.raw(PANEL_BUTTON_OFF)
...
#             header = GS + b"v0" + bytes([density_byte) + self._int_low_high(im.width_bytes, 2) +\
#                 self._int_low_high(im.height, 2)
#             self.raw(header + im.to_raster_format())
...
#             tone = b'0'
#             colors = b'1'
#             ym = bytes([1 if high_density_vertical else 2)
#             xm = bytes([1 if high_density_horizontal else 2)
...
#             header = ESC + b"*" + bytes([density_byte) + self._int_low_high(im.width, 2)
#             outp = [ESC + b"3" + bytes([16)]  # Adjust line-feed size
#             for blob in im.to_column_format(high_density_vertical):
#                 outp.append(header + blob + b"\n")
#             outp.append(ESC + b"2")  # Reset line-feed size
#             self.raw(b''.join(outp))
...
#         self.raw(GS + b'(L' + header + m + fn + data)
...
#             python_qr_ec = {
#                 QR_ECLEVEL_H: qrcodeconstants.ERROR_CORRECT_H,
#                 QR_ECLEVEL_L: qrcodeconstants.ERROR_CORRECT_L,
#                 QR_ECLEVEL_M: qrcodeconstants.ERROR_CORRECT_M,
#                 QR_ECLEVEL_Q: qrcodeconstants.ERROR_CORRECT_Q
#             }
...
#         self._send_2d_code_data(bytes([65), cn, bytes([48 + model) + bytes([0))
#         # Set dot size.
#         self._send_2d_code_data(bytes([67), cn, bytes([size))
#         # Set error correction level: L, M, Q, or H
#         self._send_2d_code_data(bytes([69), cn, bytes([48 + ec))
#         # Send content & print
#         self._send_2d_code_data(bytes([80), cn, content.encode('utf-8'), b'0')
#         self._send_2d_code_data(bytes([81), cn, b'', b'0')
...
#         self.raw(GS + b'(k' + header + cn + fn + m + data)
...
#         Sets the control sequence from ``CHARCODE`` in :py:mod:`escposconstants` as active. It will be sent with
...
#         # Align Bar Code()
#         if align_ct:
#             self.raw(TXT_STYLE['align']['center'])
#         # Height
#         if 1 <= height <= 255:
#             self.raw(BARCODE_HEIGHT + bytes([height))
#         else:
#             raise BarcodeSizeError("height = {height}".format(height=height))
#         # Width
#         if 2 <= width <= 6:
#             self.raw(BARCODE_WIDTH + bytes([width))
#         else:
#             raise BarcodeSizeError("width = {width}".format(width=width))
#         # Font
#         if font.upper() == "B":
#             self.raw(BARCODE_FONT_B)
#         else:  # DEFAULT FONT: A
#             self.raw(BARCODE_FONT_A)
#         # Position
#         if pos.upper() == "OFF":
#             self.raw(BARCODE_TXT_OFF)
#         elif pos.upper() == "BOTH":
#             self.raw(BARCODE_TXT_BTH)
#         elif pos.upper() == "ABOVE":
#             self.raw(BARCODE_TXT_ABV)
#         else:  # DEFAULT POSITION: BELOW
#             self.raw(BARCODE_TXT_BLW)

#         self.raw(bc_types[bc.upper()])

#         if function_type.upper() == "B":
#             self.raw(bytes([len(code)))

#         # Print Code
#         if code:
#             self.raw(code.encode())
#         else:
#             raise BarcodeCodeError()
...
#         # TODO flush? or on print? (this should prob rather be handled by the raw-method)
#         for line in lines:
